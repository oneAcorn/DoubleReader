 ...
 */
class DownloadUtil(val context: Context,
    ...
    fun download(url: String, callback: (downloadID: Long?) -> Unit) {
        DownloadManager.Request(Uri.parse(url)).apply {
            ...
            val file = File(context.getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS), downloadConfig.downloadApkName)
            if (file.exists()) {
                file.delete()
            }
...
//                    logI("sfdownload downSize:$downSize,totalSize:$totalSize")
                ...
                DownloadManager.STATUS_SUCCESSFUL -> { //下载完成
                    ...
                    if (downloadPath != null)
                        InstallApk(context).installApk(File(downloadPath!!))
                }
    ...
    private fun notifyInstall(builder: Notification.Builder) {
        ...
        if (downloadPath != null) {
            val openintent = InstallApk(context).getInstallIntent(File(downloadPath!!))
            val contentIntent = PendingIntent.getActivity(context, 0, openintent, 0) //当点击消息时就会向系统发送openintent意图
    ...
    private fun getNotifyInstallBuilder(): Notification.Builder {
        ...
        builder
                .setLargeIcon(BitmapFactory.decodeResource(context.resources, downloadConfig.largeIcon
                        ?: 0)) //设置下拉菜单中的图标
    ...
    private fun createAndGetchannelId(manager: NotificationManager): String {
        ...
        val channel = NotificationChannel(channelID, downloadConfig.downloadChannelTitle
